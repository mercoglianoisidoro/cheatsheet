...
It is possible to bypass the Service altogether and go directly to the Pod. 

Use it when you don't need load-balancing and you need a single Service IP
...
This will remove the IP address from the Service: the DNS provide communication  directly to the Pod.
This is needed in the case of a database cluster, where only one of the database copies is responsible for writing to the database, and all others are only allowed to read.
#### With selectors
