...
Decoupled objects need to relate to one another: such relationships are set by labels
...
When applying labels to deployments (e.g., kubectl label deployments nginx "prod=true"), only the labels for the deployment object are changed, not the ReplicaSets and Pods it creates.
...
They are defined in the metadata section, here an example:
