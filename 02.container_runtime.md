...
- The container's configuration (includes container's root filesystem, command to run, environment variables, the user and group to use, resource limits, etc.)
...
- A manifest (describes the image, lists all the other components)
...
**Layers** are TAR archives that include files and directories. The specification defines different media types for layers, including uncompressed layers, gzipped layers, and nondistributable layers.
